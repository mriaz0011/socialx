...
#import "Socialx-Swift.h"

static CGFloat const DEFAULT_MASK_ALPHA = 0.75;
static bool const square = NO;
float IMAGE_MIN_HEIGHT = 67;
float IMAGE_MIN_WIDTH = 67;
...
@synthesize delegate;
@synthesize cropView;
@synthesize updateIconImageView;
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        //Getting main screen width and height.
        CGFloat mainScreenwidth = [UIScreen mainScreen].bounds.size.width;
        CGFloat mainScreenheight = [UIScreen mainScreen].bounds.size.height;
        UILabel *topPanelLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, mainScreenwidth, 80)];
        topPanelLabel.backgroundColor = [UIColor colorWithRed:0.98 green:0.67 blue:0.09 alpha:1.0];
        [contentView addSubview:topPanelLabel];
        
        UIButton *backButton = [UIButton buttonWithType: UIButtonTypeCustom];
        backButton.frame = CGRectMake(11, 40, 45, 25);
        [backButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        //UIImage* imageBack = [UIImage imageNamed:@"Back-100.png"];
        ...
        cropButton.frame = CGRectMake(mainScreenwidth - 76.0, 40, 65, 25);
        [cropButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        //UIImage* imageCrop = [UIImage imageNamed:@"Crop-100.png"];
        //[cropButton setBackgroundImage:imageCrop forState:UIControlStateNormal];
        [cropButton setTitle:@"Preview" forState:UIControlStateNormal];
        [cropButton addTarget:self action:@selector(preview) forControlEvents:UIControlEventTouchUpInside];
        [contentView addSubview:cropButton];
        
        //Setting updateIconImageView.
        self.updateIconImageView = [[iconImageView alloc] initWithFrame:CGRectMake(11, mainScreenheight - 78,67,67)];
        self.updateIconImageView.image= _image; //[UIImage imageNamed:@"wheel.png"];
        [contentView addSubview:updateIconImageView];
        [self.view bringSubviewToFront:self.updateIconImageView];
        //Applying border and circle, but now it is flat image no corner radius
        if ([self.cropImageBoundryValue   isEqual: @"Circle"]) {
            [self.updateIconImageView.layer setCornerRadius:self.updateIconImageView.frame.size.width/6];
        } else if ([self.cropImageBoundryValue  isEqual: @"Square"]) {
            [self.updateIconImageView.layer setCornerRadius:self.updateIconImageView.frame.size.width/6];
        }
        [self.updateIconImageView.layer setMasksToBounds:YES];
        [self.updateIconImageView.layer setBorderColor: [[UIColor colorWithRed:249.0/255.0 green:172.0/255.0 blue:24.0/255.0 alpha:1] CGColor]];
        [self.updateIconImageView.layer setBorderWidth: 1.0];
    }
    
    
    
    /*//Adding nav bar programatically.
     UINavigationBar *navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, 0, 414, 48)];
     UINavigationItem *navItem = [[UINavigationItem alloc] init];
     navItem.title = @"Cropping Image";
     //Nav bar things.
     UIImage* imageBack = [UIImage imageNamed:@"Back-100.png"];
     CGRect frameImageBack = CGRectMake(0, 0, 25, 25);
     UIButton *backButton = [[UIButton alloc] initWithFrame:frameImageBack];
     [backButton setBackgroundImage:imageBack forState:UIControlStateNormal];
     [backButton addTarget:self action:@selector(done)
     ...
     UIImage* imageCrop = [UIImage imageNamed:@"Crop-100.png"];
     CGRect frameImageCrop = CGRectMake(0, 0, 25, 25);
     UIButton *cropButton = [[UIButton alloc] initWithFrame:frameImageCrop];
     [cropButton setBackgroundImage:imageCrop forState:UIControlStateNormal];
     [cropButton addTarget:self action:@selector(preview)
     forControlEvents:UIControlEventTouchUpInside];
     [cropButton setShowsTouchWhenHighlighted:YES];
     UIBarButtonItem *rightbutton =[[UIBarButtonItem alloc] initWithCustomView:cropButton];
     //self.navigationItem.rightBarButtonItem = rightbutton;
     navItem.rightBarButtonItem = rightbutton;
     navBar.items = @[ navItem ];
     //navBar.backgroundColor = [UIColor colorWithRed:0.98 green:0.67 blue:0.09 alpha:1.0];
     navBar.barTintColor = [UIColor colorWithRed:0.98 green:0.67 blue:0.09 alpha:1.0];
     //self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:0.98 green:0.67 blue:0.09 alpha:1.0];
...
- (void)done
{
    ...
    if (self.image != nil){
        ...
        updateIconImageView.image = cropped;
        /*AddSiteViewController * myOb = [AddSiteViewController new];*/
        NSString * croppedImageName = @"";
        //Now it is flat image no corner radius
        if ([self.cropImageBoundryValue   isEqual: @"Circle"]) {
            //croppedImageName = [myOb savingImageLocally:cropped];
            croppedImageName = [RepeatMethods savingImageLocallyWithSelectedImage:cropped];
        } else if ([self.cropImageBoundryValue  isEqual: @"Square"]) {
            croppedImageName = [RepeatMethods savingImageLocallyWithSelectedImage: cropped];
        }
        NSString * check = @"true";
        NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
        [standardUserDefaults setObject:check forKey:@"check"];
        [standardUserDefaults setObject:croppedImageName forKey:@"croppedImageName"];
        [standardUserDefaults synchronize];
        //NSLog(croppedImageName);
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.color = [UIColor colorWithRed:249.0/255.0 green:172.0/255.0 blue:24.0/255.0 alpha:1]; //Crop area's four selection points color.
        self.opaque = NO;
    }
    return self;
}
...
- (void)initViews {
    ...
    cropAreaView.layer.borderWidth = 1; //Crop area border size.
    cropAreaView.layer.borderColor = [[UIColor blackColor] CGColor]; //Crop area border color.
...
- (void)setControlColor:(UIColor *)_color {
    ...
    self.shadeView.layer.borderWidth = 2;
